...
$.tale.prototype.alertOk = function (options) {
    ...
    options.title = options.title || 'Hoạt động thành công';
...
$.tale.prototype.alertWarn = function (options) {
    options = options.length ? {text:options} : ( options || {} );
    options.title = options.title || 'Tin nhắn cảnh báo';
    options.text = options.text;
    options.timer = 3000;
...
$.tale.prototype.alertConfirm = function (options) {
    options = options || {};
    options.title = options.title || 'Bạn chắc chắn muốn xoá nó？';
    options.text = options.text;
...
$.tale.prototype.alertError = function (options) {
    options = options.length ? {text:options} : ( options || {} );
    options.title = options.title || 'Thông báo lỗi';
    options.text = options.text;
    options.type = 'error';
    this.alertBox(options);
};
...
$.tale.prototype.alertBox = function (options) {
    swal({
        ...
        confirmButtonText: options.confirmButtonText || 'Chắc chắn',
        cancelButtonText: options.cancelButtonText || 'Từ chối'
    }).then(function (e) {
